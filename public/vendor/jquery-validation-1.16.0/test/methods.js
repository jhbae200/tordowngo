...
/**
 * Creates a dummy DOM filesave input with FileList
 * @param filename
 * @param mimeType
 * @returns {{}}
 */
...
QUnit.test( "filesave accept - image wildcard", function( assert ) {
	...
	assert.ok( proxy(), "the selected filesave for upload has specified mime type" );
} );

QUnit.test( "filesave accept - specified mime type", function( assert ) {
...
QUnit.test( "filesave accept - multiple mimetypes", function( assert ) {
	var input = acceptFileDummyInput( "test.png", "image/png" ),
		$form = $( "<form />" ),
...
QUnit.test( "filesave accept - multiple mimetypes with wildcard", function( assert ) {
...
QUnit.test( "filesave accept - invalid mime type", function( assert ) {
	var input = acceptFileDummyInput( "test.kml", "foobar/vnd.google-earth.kml+xml" ),
		$form = $( "<form />" ),
		proxy = $.proxy( $.validator.methods.accept, new $.validator( {}, $form[ 0 ] ), null, input, "application/vnd.google-earth.kml+xml" );
	assert.equal( proxy(), false, "the selected filesave for upload has invalid mime type" );
} );
