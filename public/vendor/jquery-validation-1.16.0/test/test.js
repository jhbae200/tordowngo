...
QUnit.test( "elementValue() returns the filesave input's name without the prefix 'C:\\fakepath\\' ", function( assert ) {
	var v = $( "#userForm" ).validate(),

		// A fake filesave input
		fileInput = {
			name: "fakeFile",
			type: "file",
			files: {},
	...
	assert.ok( v.element( fileInput ), "The fake filesave input is valid (length = 12, minlength = 10)" );

	fileInput.value = "C:\\fakepath\\filesave.txt";
	assert.ok( !v.element( fileInput ), "The fake filesave input is invalid (length = 8, minlength = 10)" );
	...
	assert.ok( v.element( fileInput ), "The fake filesave input is valid (length = 8, maxlength = 10)" );

	fileInput.value = "C:\\fakepath\\fakefile.txt";
	assert.ok( !v.element( fileInput ), "The fake filesave input is invalid (length = 12, maxlength = 10)" );
} );
