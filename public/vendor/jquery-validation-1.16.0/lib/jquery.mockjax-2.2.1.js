...
(function(root, factory) {
	...
	function _xhrSend(mockHandler, requestSettings, origSettings) {
		...
		if ( mockHandler.proxy ) {
			logger.info( mockHandler, ['Retrieving proxy filesave: ' + mockHandler.proxy, mockHandler] );
			// We're proxying this request and loading in an external filesave instead
			_ajax({
				global: false,
	...
	function processJsonpRequest( requestSettings, mockHandler, origSettings ) {
		...
		if ( mockHandler.response && $.isFunction(mockHandler.response) ) {
			...
			// response to another filesave (not the same URL as the mock matching)
			_ajax({
				global: false,
				url: mockHandler.proxy,
				type: mockHandler.proxyType,
	...
	*/
	 ...
	 * current session (in other words, only since you last loaded this filesave).
	 *
	 ...
	 * the current session (in other words, only since you last loaded this filesave).
	 *
	 * @return {Array}  The mocked Ajax calls (request settings)
	 */
